...
def compute_weight_penalty(weights, ones):
...
def train_network(N, data, targets, iterations, lr):
    ...
    # Set up weights
    w_hidden = tf.Variable(np.array(np.random.uniform(0.0, 1.0, (2**N, 2*N + 1))), name='w_hidden')
    w_out = tf.Variable(np.random.uniform(0.0, 1.0, 2**N + 1), name='w_out')
    ...
    # Compute error
    penalty = 0.02 * (compute_weight_penalty(w_hidden, 2*N) + compute_weight_penalty(tf.transpose(tf.expand_dims(w_out, 1)), 2**N))
    ...
    with tf.Session() as session:
        ...
        for i in range(iterations):
            ...
            session.run(train_op_penalty, feed_dict={r: float(i)/float(iterations)})
            session.run(clip_op_hidden)
            session.run(clip_op_out)
