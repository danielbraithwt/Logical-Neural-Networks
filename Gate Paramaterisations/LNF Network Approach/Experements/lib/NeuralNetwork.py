...
def train_perceptron_network_general(N, data, targets, iterations, num_out=1):
    ...
    with tf.device("/cpu:0"):
        ...
        y_hat_prime = tf.reduce_sum(y_hat, axis=1)
    ...
    with tf.Session() as session:
        session.run(model)
        #print(session.run(y_hat, feed_dict={x:data, y:targets}))
        #print(session.run(y_hat_prime, feed_dict={x:data, y:targets}))

        #print(session.run(y, feed_dict={x:data, y:targets}))
        ...
        error = session.run(error, feed_dict={x:data, y:targets})
        w_hidden_value = session.run(w_hidden)
        b_hidden_value = session.run(b_hidden)[0]

        w_l1_value = session.run(w_l1)
        b_l1_value = session.run(b_l1)[0]
