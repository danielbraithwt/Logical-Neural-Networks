...
if __name__ == '__main__':
    ...
    np.random.seed(1234)
    random.seed(1234)
    expressions = generateExpressions(n, 1)

    for e in expressions:
        data = e[0]
        targets = e[1]

        r_net_cnf, _, er_cnf, _ = train_cnf_network(n, data, targets, 300000)
        print("CNF Trained: ", er_cnf)
        r_net_dnf, _, er_dnf, _ = train_dnf_network(n, data, targets, 300000)
